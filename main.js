...
function start() {
  ...
  letters[
    count
  ] = `<span id="test-cursor"></span><span class="typed-letter">${letters[count]}</span>`;
...
function textCheck(keyValue) {
  ...
  if (time !== 0) {
    if (keyPressed == para[0][count]) {
      ...
      letters[
        count + 1
      ] = `</span><span id="test-cursor"></span><span class="typed-letter">${
        letters[count + 1]
      }</span>`;
      element.innerHTML = letters.join("");
      count++;
      ...
      letters[
        count
      ] = `</span><span id="test-cursor"></span><span class="wrong-letter">${letters[count]}</span>`;
      element.innerHTML = letters.join("");
      wrong++;
    }
  }
...
function showReview() {
  ...
  document.getElementById(
    "your-speed"
  ).innerHTML = `<span class="review-heading">WPM</span>${speed}`;
  document.getElementById(
    "your-accuracy"
  ).innerHTML = `<span class="review-heading">acc</span>${accuracy}%`;
  document.getElementById(
    "timer"
  ).innerHTML = `<span class="review-heading">time</span>${startTime}`;
  document.getElementById(
    "your-raw-speed"
  ).innerHTML = `<span class="review-heading">raw</span>${raw}`;
  document.getElementById(
    "your-characters"
  ).innerHTML = `<span class="review-heading">characters</span>${total}/${count}/${wrong}`;
}
